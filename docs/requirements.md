...
| 10 | 初始化 | 支持 `stages init`（可选 `-m`，默认 commit 名 `init`）；首次 `stages` 也可自动初始化；**扫描工作区写入 baseline stages commit**，不自动建 stage、不改写工作区 |
...
| **初始化**（`stages init`） | 创建存储与初始元数据；`.stages/` 写入 `.gitignore`；记录 git HEAD；**将当前工作区钉为 baseline stages commit**（`-m` 可命名，默认 `init`）；不创建 stage、不改写工作区 |
